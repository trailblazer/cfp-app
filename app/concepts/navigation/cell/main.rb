...
module Navigation
  module Cell
    class Main < Trailblazer::Cell
      ...
      def current_event
        context[:current_event] || current_user.proposals.first.event
      end
      ...
      def display_staff_event_subnav?
        %w[TeammatesController EventsController].include?(controller_name)
      end

      def schedule_mode?
        %w[ProgramSessionsController BulkTimeSlotsController TimeSlotsController RoomsController].include?(controller_name)
      end

      def program_mode?
        %w[GridsController ProposalsController SpeakersController].include?(controller_name)
      end
      ...
      def exact_path(sym, *deps)
        context[:controller].send(sym.to_s + '_path', *deps) unless deps.include?(nil) # don't generate the path unless all dependencies are present
      end
      ...
      def match?(paths)
        # TODO: check if this still works with request
